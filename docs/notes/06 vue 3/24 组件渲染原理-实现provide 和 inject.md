# 简介
+ 在 Vue 3 中，[**provide 和 inject**](https://cn.vuejs.org/guide/components/provide-inject.html) 是一对用于**跨层级组件通信**的 API，主要用于解决深层嵌套组件之间的数据传递问题，避免繁琐的 `props` 逐层传递。

## 核心概念
1. `**provide**`** ****(提供)**
    - 在**祖先组件**中定义数据或方法，并向下提供给所有子孙组件。
    - 可以是任意类型的数据（如响应式对象、函数等）。
2. `**inject**`** ****(注入)**
    - 在**子孙组件**中接收祖先组件提供的数据或方法。
    - 支持默认值和类型校验。

## 作用
1. **跨层级通信**
    - 解决多级嵌套组件（如祖孙组件）间的数据传递问题，避免中间组件冗余的 `props` 声明。
2. **状态共享**
    - 替代 Vuex/Pinia 的轻量级方案，适合局部状态管理（如主题切换、用户权限等）。
3. **逻辑复用**
    - 通过注入函数或工具方法，实现逻辑的跨组件复用。
...
## 高级特性
### 响应式数据
+ 如果提供的是 `ref` 或 `reactive` 对象，注入的组件会自动响应变化
...
| --- | --- | --- |
| **方向** | 祖先 → 子孙（跨层级） | 父 → 子（直接传递） |
| **响应式** | 支持 | 支持 |
| **适用场景** | 深层嵌套组件 | 父子组件简单通信 |
| **代码侵入性** | 低（无需中间组件参与） | 高（需逐层声明 `props`） |


## 注意事项
1. **避免滥用**
    - 复杂全局状态建议使用 Pinia/Vuex，`provide/inject` 更适合局部共享。
2. **命名冲突**
    - 使用唯一的 key（如 Symbol）避免命名冲突：
...
# 基本使用
```typescript
import { ref, render, h, provide, inject, reactive, getCurrentInstance } from 'vue'

const GrandChild = {
  setup() {
    console.log('grand child setup: ', getCurrentInstance())
    const message = inject('message')
    const age = inject('age')
    const name = inject('name', 'no name')
    const city = inject('city', 'no city')
    const job = inject('job', () => 'no job', true)
    return () => h('div', {class: 'grand-child'}, [
      h('p', {}, 'message to grand child:' + message?.value),
      h('p', {}, 'age to grand child:' + age?.value),
      h('p', {}, 'name to grand child:' + name),
      h('p', {}, 'city to grand child:' + city),
      h('p', {}, 'job to grand child:' + job),
    ])
  }
}
...
const App = {
  setup() { 
    console.log('app setup: ', getCurrentInstance())
    const message = ref('hello world')
    const state = reactive({
      message: 'hello world'
    })
    // 这里的message是响应式的，但是子组件拿到的message不是响应式的， 因为provide的参数是浅的
    // provide('message', state.message) 
    provide('message', message)
    provide('name', 'Join') // 被子组件的provide覆盖了

    return () => h('div', {class: 'app'}, [
      h('input', {value: message.value, onInput(e) {
        // console.log(state)
...
<body>
  <div id="app"></div>
  <script type="module">
    // import { ref, render, h, provide, inject, reactive, getCurrentInstance } from '../../node_modules/vue/dist/vue.esm-browser.js'
    import { ref, render, h, provide, inject, reactive, getCurrentInstance } from '../../packages/vue/dist/vue.esm.js'
