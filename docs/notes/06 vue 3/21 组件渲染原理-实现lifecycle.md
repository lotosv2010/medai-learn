# 简介
+ 在 Vue 3 中，**生命周期（Lifecycle）** 是指组件从创建到销毁的整个过程，每个阶段会触发特定的钩子函数（Lifecycle Hooks）。开发者可以通过这些钩子函数在组件的不同阶段执行自定义逻辑。

## Vue 3 生命周期钩子函数
+ 以下是 Vue 3 的主要生命周期钩子函数及其作用：

### 创建阶段
+ `**setup**`
    - **作用**：Composition API 的入口，在组件初始化时执行，用于定义响应式数据、计算属性、方法等。
    - **注意**：`setup` 中没有 `this`，因为它是在组件实例创建之前调用的。
+ `**onBeforeMount**`
    - **作用**：在组件挂载到 DOM 之前触发，此时模板已编译但尚未渲染到页面。
+ `**onMounted**`
    - **作用**：组件挂载到 DOM 后触发，此时可以访问 DOM 元素或执行依赖 DOM 的操作（如初始化第三方库）。

### 更新阶段
+ `**onBeforeUpdate**`
    - **作用**：在组件数据变化后、DOM 重新渲染之前触发。适合在更新前访问旧的 DOM 状态。
+ `**onUpdated**`
    - **作用**：DOM 重新渲染完成后触发。避免在此钩子中修改状态，否则可能导致无限循环。

### 卸载阶段
+ `**onBeforeUnmount**`
    - **作用**：组件卸载前触发，适合清理定时器、取消事件监听等操作。
+ `**onUnmounted**`
    - **作用**：组件卸载后触发，所有绑定的事件和子组件都会被移除。

### 其他钩子
+ `**onErrorCaptured**`
    - **作用**：捕获子组件或当前组件的错误，可用于错误上报或降级处理。
+ `**onActivated**`** ****/**** **`**onDeactivated**`
    - **作用**：与 `<keep-alive>` 结合使用，组件被激活或停用时触发。
...
1. **Composition API 与 Options API 的区别**
    - Vue 3 推荐使用 Composition API（如 `setup` + `onXxx`），但 Options API（如 `mounted`）仍然兼容。
    - Composition API 的钩子函数需要显式导入（如 `import { onMounted } from 'vue'`）。
2. **异步操作**
    - 在 `onMounted` 或 `onUpdated` 中执行异步操作时，需注意避免内存泄漏（如未清理的定时器）。
3. **与**** **`**<script setup>**`** ****语法糖结合**
    - 在 `<script setup>` 中，可以直接使用生命周期钩子：****
...
# 基本使用
```typescript
import { ref, render, h, Text, getCurrentInstance, onBeforeMount, onMounted, onBeforeUpdate, onUpdated, onBeforeUnmount, onUnmounted } from 'vue'
