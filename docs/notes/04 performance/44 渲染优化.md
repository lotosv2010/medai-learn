...
+ 浏览器从获取 HTML 到最终在屏幕上显示内容需要完成以下步骤：
    ...
    - 将各个节点绘制到屏幕上。
+ 经过以上整个流程我们才能看见屏幕上出现渲染的内容，**<font style="color:#DF2A3F;">优化关键渲染路径就是指最大限度缩短执行上述第 1 步至第 5 步耗费的总时间</font>**，让用户最快的看到首次渲染的内容。
...
<!-- 这是一张图片，ocr 内容为：合成 绘制 计算样式 JAVASCRIO+处理 页面布局 拉勾教育 -->
...
## 优化 JavaScript 的使用
+ 所有文本资源都应该让**文件尽可能的小**，JavaScript 也不例外，它也需要删除未使用的代码、缩小文件的尺寸（Minify）、使用 gzip 压缩（Compress）、使用缓存（HTTP Cache）。
    - 异步加载 JavaScript
    - 避免同步请求
    - 延迟解析 JavaScript
    - 避免运行时间长的 JavaScript
...
> 注意：defer 特性仅适用于外部脚本，如果 \x3Cscript> 脚本没有 src，则会忽略 defer 特性。
>
...
## 参考链接
+ [前端性能优化之关键路径渲染优化 · Issue #16 · fi3ework/blog](https://github.com/fi3ework/blog/issues/16)
+ [关键渲染路径 · Issue #29 · berwin/Blog](https://github.com/berwin/Blog/issues/29)
+  [详解 CRP：如何最大化提升首屏渲染速度 - 掘金](https://juejin.cn/post/6844903757038223367#heading-4)
...
<!-- 这是一张图片，ocr 内容为：定时器触发 阻止刷新 JAVASCRIPT处理 RS合成 16MS 16MS 16MS 拉勾教育 -->
...
<!-- 这是一张图片，ocr 内容为：FIREFOX FOR ANDROID SAMSUNG INTERNET WEBVIEW ANDROID CHROME ANDROID OPERA ANDROID SAFARI ON IOS CHROME FIREFOX OPERA SAFARI EDGE O O 八 V 7 7 1.5 15 23 14 4.4 25 24 23 12 REQUESTANIMATIONFRAME -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672986612758-980cb62e-88eb-4915-80bd-f74de23e56f6.png)
...
<!-- 这是一张图片，ocr 内容为：DEVTOOLS-DEVELOPER.MOZILLA.ORG/ZH-CN/DOCS/WEB/API/HTMLELEMENT/OFFSET ENT/OFFSETTOP MEMORYAPPLICATIONSECURITY ELEMENTS 143 CONSOLE NETWORK AUDITS SOURCES PERFORMANCE COMPONENTS CO SCREENSHOTS MEMORY DEVELOPER.MOZILLA.ORG #1 DISABLE JAVASCRIPT SAMPLES NETWORK:ONLINE ENABLE ADVANCED PAINT INSTRUMENTATION(SLOW) CPU:NO THROTTLING 35000MS 25000MS 10000M: 5000MS 30000 MS 40000 15000 MS 20000 MS FPS CPU NET 7000MS 3000MS 8000MS 5000MS 9000MS 4000 MS 6000MS 1000C NETWORK PRINT.C+WHOAMI(DE... FRAMES INTERACTIONS ANIMATION TIMINGS DCL MAIN-HTTPS://DEVELOPER.MOZILLA.ORG/ZH-CN/DOCS/WEB/API/HTMLELEMENT/OFFSETTOP T... E.... SUMMARY BOTTOM-UP CALL TREE EVENT LOG EVALUATE SCRIPT TOTAL TIME 209.27 MS SELF TIME 8.47 MS SCRIPT CONTENTSCRIPT.JS:1 AGGREGATED TIME SCRIPTING(SELF) 8 MS SCRIPTING(CHILDREN) 199MS 209 MS 1 MS SYSTEM 拉勾教育 TOTAL 209 MS -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672998601184-a77c047c-6c8c-4873-a5bc-ec9ab1b70f2d.png)

<!-- 这是一张图片，ocr 内容为：DEVTOOLS-MEI.YOUZAN.COM/DASHBOARD SOURCES MEMORY PERFORMANCE ELEMENTS CONSOLE NETWORK APPLICATION JAVASCRIPT PROFILERX HEAVY (BOTTOM UP) O X SELF TIME FUNCTION TOTAL TIME PROFILES 13585.9 MS 13585.9 MS (IDLE) CPU PROFILES 4827.6MS  68.18% 4827.6MS  68.18% (PROGRAM) 928.8MS 13.12% 928.8MS 13.12% (GARBAGE COLLECTOR) PROFILE 1 SAVE 152.8MS 2.16% 210.9 MS 2.98% VNA YENDOR DLL 7394...0165FF22C.JS:1 147.6MS  2.08% 2.89% 204.8 MS VENDOR DLL 7394...0165FF22C.JS:1 78.8 MS 1.11% 1.50% 106.5MS VENDOR DLL 7394...0165FF22C.JS:1 VC VENDOR DLL 7394...0165FF22C.JS:1 1.05% YH 55.9MS 0.79% 74.7 MS VENDOR DLL 7394...0165FF22C.JS:1 22.9MS 0.32% 0.45% 31.9MS VENDOR DLL 7394...0165FF22C.JS:1 86.8MS  1.23% 61.2MS 0.86% 7.8MS 0.11% 5.5MS 0.08% YENDOR DLL 7394...0165FF22C.JS:1 3.7MS 0.05% 2.1MS 0.03% YENDOR DLL 7394...0165FF22C.JS 5.7MS 0.08% 4.9MS 0.07% $U VENDOR DLL 7394...0165FF22C.I 0.4MS 0.01% 0.4MS 0.01% LA VENDOR DLL  7394...0165FF22C.IS: 132.7MS 1.87% 132.7MS 1.87% VENDOR DLL 7394F (ANONYMOUS) -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672998622853-a5695846-da04-47ce-8774-7b6091573457.png)
...
## 减少要计算样式的元素数量
+ 首先我们需要知道与计算样式相关的一条重要机制：**<font style="color:#DF2A3F;">CSS 引擎在查找样式表时，对每条规则的匹配顺序是从右向左的</font>**，这与我们通常从左向右的书写习惯相反。举个例子，如下 CSS 规则：
...
<!-- 这是一张图片，ocr 内容为：APPLICATION PERFORMANCE NETWORK ELEMENTS WELCOME SOURCES MEMORY CONSOLE 默认级别 1个问题 FILTER TOP X 渲染 CONSOLE 画图闪烁 "突出显示需要重新绘制的页面区域(绿色).可能不适合易发光敏性癫病的人士. 布局移动区域 ,突出显示已移动的页面区域(蓝色).可能不适合易发光敏性癫病的人士. 层边框 显示图层边框(橙色/橄榄色)和磁贴(青色). 帧渲染统计信息 绘制帧吞吐量,丢帧分布和GPU内存. 滚动性能问题 (突出显示可能会减慢滚动速度的元素(青色),包括触控和滚轮事件处理程序以及其他主线程滚动情况. 突出显示广告帧 突出显示已检测为广告的帧(红色). 命中测试边框 显示命中测试区域周围的边框. 核心网页指标 显示具有核心网页指标的覆盖. 禁用本地字体 在@FONT-FACE 规则中禁用本地0源.需要重新加载页面才能应用. 模拟焦点页面 模拟焦点页面. 模拟CSS媒体类型 强制测试打印和屏幕样式的媒体类型 没有模拟 模拟CSS媒体功能首选颜色方案 强制CSS首选颜色方案媒体功能 没有模拟 模拟CSS媒体功能首选减少运动 强制CSS 首选减少运动媒体功能 没有模拟 仿真CSS媒体功能颜色域 强制CSS颜色域媒体功能 没有模拟 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673000230221-2291aba7-fe08-40ef-9f58-495e69ceba0d.png)
