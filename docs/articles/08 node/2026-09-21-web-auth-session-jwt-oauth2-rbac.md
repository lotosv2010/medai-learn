...
- **你是谁** —— 登录方式全景（密码、验证码、扫码、SSO、API Key……到底有多少种入口）＋ 密码怎么安全存储
- **怎么证明** —— 认证凭证三选一（Cookie 自动携带 / Session 有状态 / JWT 无状态）及其安全纵深（token 怎么管、会话怎么防攻击）
- **怎么委托** —— OAuth2 让第三方应用「代表用户」办事，四种授权模式与 OIDC 的关系
- **能做什么** —— 权限模型（RBAC/ABAC/ACL/DAC/MAC）与前后端权限联动

全文按这条四环主线展开，每一环内部都走完「怎么用 → 为什么 → 源码 → 手写」的纵向递进。一张地图先给你：

| 四环主线 | 章节 | 一句话内核 |
|---------|------|-----------|
| 你是谁 | 一 | 登录方式全景 + 密码哈希存储（加盐 + 慢哈希） |
| 怎么证明 | 二 | Cookie / Session / JWT 三选一 + 双令牌 + 三大攻击面 |
| 怎么委托 | 三 | OAuth2 四种授权模式 + 授权码流程源码 |
| 能做什么 | 四 | RBAC 到 ABAC/ACL/DAC/MAC + 前后端联动 |
...
再把这张全景地图收束回两个正交的维度，就不会把「登录方式」和「认证方案」混为一谈：

- **验证方式**（怎么证明「你是谁」）：知识（密码）、持有（验证码/设备）、固有（生物）、委托（OAuth2/SSO）
- **凭证载体**（怎么携带「已认证」）：Cookie 自动携带 / token 手动携带

密码登录、验证码登录、扫码登录最终可能落到 Session（Cookie），也可能落到 JWT（token）；API Key、第三方登录几乎必然落到 token。**登录方式决定「怎么验证」，凭证载体决定「怎么携带」**——这是理解后面所有内容的钥匙。
...
`bcrypt` 的两个核心设计——**加盐（salt）**和**慢哈希（cost factor）**——在下面第 3、4 节讲透，这里先记住用法。
...
## 二、怎么证明你是谁：Cookie / Session / JWT 与安全纵深

「验证了你是谁」之后，接下来的问题变成「怎么在后续每个请求里证明你还是你」。这一环是全文最重的部分，节奏是：**Cookie 全链路 → Session 全链路 → JWT 全链路 → 跨凭证选型 → 安全纵深**，每个凭证都走完「使用 → 原理 → 源码 → 手写」。

### 1. Cookie：Set-Cookie 与 document.cookie
...
### 3. cookie-parser 的 sign/unsign 签名机制（源码）
...
`sign` 的格式是 `原始值 + '.' + HMAC-SHA256(原始值, 密钥)`。`unsign` 把签名部分剥掉，用同一个密钥重新算一遍摘要，如果和客户端带过来的签名一致，说明值没被改过（因为攻击者没有密钥，算不出正确的签名）。这就是「签名防篡改」的最小实现，和下面第 10 节 JWT 的签名原理**完全同源**。
...
### 6. express-session 的 Store 抽象接口（源码）
...
### 7. packages/mini-session：手写 Session 中间件
...
- **难主动失效**：JWT 签发后不像 Session 能直接删存储记录，要失效得靠额外机制（见第 12 节）

### 10. jsonwebtoken 的 sign/verify 核心（源码）
...
### 11. packages/mini-jwt：手写 JWT 签发与验证

HMAC-SHA256 签名 + base64url 编解码 + `exp` 校验，不依赖第三方库（核心代码见上一节）。补一个**正确的 base64url 实现**（标准库手写时最容易错的地方）：
...
### 3. OAuth2 授权码流程：code 换 token 的完整链路（源码）
...
权限校验的核心是中间件 `checkUser`（第二章第 4 节已给出 Session 版，JWT 版同理），判断当前用户拥有的角色是否包含访问该接口所需的权限编码。

### 2. 授权模型横向对比：RBAC 与它的兄弟们
...
| 你是谁 | 密码存储 | 哈希 + 加盐 + 慢哈希，防拖库/彩虹表/暴力破解 | ⭐⭐⭐ 必考 |
...
| 怎么证明 | 双令牌 | 短 access + 长 refresh + 轮换复用检测 | ⭐⭐⭐ 必考 |
...
- https://github.com/lotosv2010/g-rbac（RBAC 通用权限系统参考实现）
- GitHub：`medai-node-source` 仓库（手写实现 `packages/mini-session`、`packages/mini-jwt`、`packages/mini-rbac`，地址待补充）
