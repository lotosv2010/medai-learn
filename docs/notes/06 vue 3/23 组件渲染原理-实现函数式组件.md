# 简介
+ 在 Vue 3 中，[**函数式组件（Functional Component）**](https://cn.vuejs.org/guide/extras/render-function.html#functional-components) 是一种特殊的组件形式，它没有自身的状态（即没有 `data`、`setup` 或响应式数据），也没有实例（`this`），仅通过接收 `props` 和 `context` 来渲染内容。它的核心特点是**无状态、轻量级、高性能**。

## 函数式组件的作用
1. **性能优化**
    - 函数式组件比普通组件更轻量，因为它没有实例化过程，适合渲染大量静态内容或纯展示型组件（如列表项、按钮等）。
2. **简化逻辑**
    - 当组件只需要根据 `props` 渲染 UI，不需要响应式数据或生命周期时，可以用函数式组件减少代码复杂度。
3. **复用性**
    - 适合作为高阶组件（HFC）或渲染逻辑的封装工具（例如动态生成多个相似组件）。

## Vue 3 中的实现方式
### Options API 写法
+ 通过 `functional: true` 标记
...
export default function FunctionalComp(props, { slots }) {
  return h('div', props.message);
}
```
...
// 直接返回 VNode
const render = () => h('div', props.message);
</script>
```

## 核心特性
1. **无实例**
    - 没有 `this`，无法访问组件实例的属性和方法（如 `$refs`、`$emit`）。
2. **仅依赖**** **`**props**`** ****和**** **`**context**`
    - `props`：接收父组件传递的数据。
    - `context`：包含 `slots`、`attrs`、`emit` 等工具。
3. **必须返回 VNode**
    - 通过 `h()` 或 JSX 返回虚拟节点（VNode）。

## 使用场景
+ **静态列表项**：如渲染一个只依赖父组件数据的列表项。
+ **高阶组件**：封装条件渲染逻辑（如 `v-if` 的抽象）。
+ **性能敏感场景**：如表格中的大量单元格渲染。

## 与普通组件的区别
| 特性 | 函数式组件 | 普通组件 |
| --- | --- | --- |
| **实例化** | 无实例（无 `this`） | 有实例（`this`） |
| **响应式数据** | 不支持 | 支持 |
| **生命周期钩子** | 不支持 | 支持 |
| **性能** | 更高（轻量级） | 较低（有实例化开销） |
...
## 注意事项
1. **避免滥用**
    - 函数式组件不适合需要内部状态或复杂交互的场景。
2. **调试困难**
    - 因为没有实例，开发者工具中可能无法直接调试。
3. **Vue 3 的兼容性**
    - Vue 3 对函数式组件的支持更灵活，但部分 Options API 的写法（如 `functional: true`）可能在未来被废弃。
  ...
  <body>
    ...
    <script type="module">
      ...
      const component = (props) => {
        return h('div', {style: {color: "red"}}, `${props.name} ${props.age}`)
      }

      render(h(component, {name: 'test', age: 18}), document.getElementById("app"))
    </script>
  </body>
</html>
