...
- **内容协商三兄弟**：HTTP 允许客户端在请求头里声明自己的偏好，服务端从多个可用表示里选出最匹配的一种返回，这套机制统称"内容协商”——`Accept-Encoding: gzip, br` 声明客户端能解压的压缩格式，服务端据此选择用 gzip 还是 Brotli（br）压缩响应体：gzip 压缩率适中但 CPU 开销低、兼容性好，br 压缩率更高（尤其对文本类资源）但压缩耗时更长，通常用于可以提前离线压缩好的静态资源（如医院 HIS 系统的药品说明书静态页面），动态生成的响应更倾向 gzip 以降低实时压缩的 CPU 成本；`Accept-Language: zh-CN,en;q=0.9` 声明语言偏好及优先级权重，服务端或 CDN 边缘节点据此返回对应语言版本内容，或结合 Cookie/URL 路径做更持久的语言路由决策；`User-Agent` 声明客户端类型（浏览器/版本/操作系统），历史上被用于"UA 嗅探"做兼容性适配或返回移动端/桌面端不同内容，但 UA 字符串可以被随意伪造，作为安全判断依据不可靠，这也是近年 User-Agent Client Hints 提案希望替代传统 UA 嗅探、让客户端主动、按需上报结构化能力信息的动机
