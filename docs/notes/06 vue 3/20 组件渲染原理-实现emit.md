# 简介
+ 在 Vue 3 中，`setup` 函数中的 `emit` 是用于**触发自定义事件**的方法，它是 Vue 组件通信的核心机制之一。`emit` 的作用是让子组件向父组件传递数据或通知父组件某些行为的发生。

## `emit` 的作用
1. **子组件向父组件通信**
    - 子组件通过 `emit` 触发自定义事件，父组件可以通过 `v-on`（或 `@`）监听这些事件并执行相应的逻辑。
2. **传递数据**
    - 子组件可以在触发事件时附带数据（如 `emit('event-name', data)`），父组件可以通过事件回调接收这些数据。
3. **解耦父子组件**
    - 子组件不需要知道父组件的具体实现，只需通过 `emit` 通知父组件发生了什么，父组件自行处理逻辑。

## `emit` 的使用方法
+ 在 `setup` 函数中，`emit` 是通过**第二个参数（上下文对象）**提供的

```javascript
setup(props, { emit }) {
  // 触发一个名为 "my-event" 的事件，并传递数据
  const handleClick = () => {
    emit('my-event', { value: 123 });
  };
...
## 关键点
### 事件命名
+ Vue 推荐使用短横线命名法（如 `my-event`），而不是驼峰命名法（如 `myEvent`），因为 HTML 属性是大小写不敏感的。

### 验证事件
+ 可以通过 `emits` 选项显式声明组件支持的事件，便于维护和调试

```javascript
export default {
  emits: ['my-event'], // 声明支持的事件
  setup(props, { emit }) {
    emit('my-event', data); // 合法
    emit('unknown-event', data); // 会触发警告
  }
}
```

### 与 `v-model` 结合
+ `emit` 常用于实现自定义组件的 `v-model`。例如：
1. **javascript**
...
### 父组件（Parent.vue）
```html
<Child @age-change="onAgeChange" />
```

```javascript
methods: {
  onAgeChange(newAge) {
    console.log(`子组件传递的年龄：${newAge}`); // 输出 99
  }
}
```
...
## 总结
+ `emit` 是子组件向父组件通信的桥梁。
+ 通过 `emits` 选项可以显式声明事件，提高代码可读性。
+ 结合 `v-model` 可以实现双向绑定的自定义组件。
  ...
  setup(props, { emit }) {
    return (proxy) => {
      return h('div', [
        h('p', proxy.name),
        h('p', proxy.foo),
        h('button', { onClick: () => emit('myClick', proxy.age) }, '点击')])
    }
  }
}

render(h(component, { onMyClick: clickHandler, foo: 'foo', bar: 'bar', style: { color: 'red' } }), app)
```
...
<body>
  ...
  <script type="module">
    ...
    render(h(component, { style: { color: 'red' } }), app)
