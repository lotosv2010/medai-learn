# 简介
+ 在 Vue 3 中，插槽（[Slots](https://cn.vuejs.org/guide/components/slots.html#slots)） 是一种组件间内容分发的机制，允许父组件向子组件传递模板片段或动态内容。插槽的核心作用是增强组件的灵活性和复用性，使组件能够接收外部定义的内容并渲染到指定位置。
+ 插槽的作用
    - 内容分发：父组件可以通过插槽将任意内容（文本、HTML、其他组件等）传递给子组件，子组件负责在特定位置渲染这些内容。
    - 解耦 UI 结构：子组件不需要知道插槽内容的具体实现，只需提供插槽的占位位置，父组件可以自由定制内容。
    - 支持作用域插槽：子组件可以通过插槽向父组件传递数据（如 {title}），父组件可以基于这些数据动态渲染内容。
+ 插槽的类型
    - 默认插槽（Default Slot）
    - 具名插槽（Named Slot）
    - 作用域插槽（Scoped Slot）
+ 插槽是 Vue 组件化的核心特性之一，适用于：
    - 封装可复用的布局组件（如卡片、模态框）。
    - 动态渲染父组件提供的内容（如列表项、表格单元格）。
    - 实现高阶组件（HOC）或渲染代理。
...
<body>
  ...
  <script type="module">
    ...
    import { ref, render, h } from '../../packages/vue/dist/vue.esm.js'
