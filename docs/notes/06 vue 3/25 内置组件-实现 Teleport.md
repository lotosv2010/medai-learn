# 简介
+ 在 Vue 3 中，[**<Teleport>**](https://cn.vuejs.org/guide/built-ins/teleport.html) 是一个**内置组件**，用于将模板中的一部分内容“传送”到 DOM 结构中的其他位置，而无需改变组件的逻辑层级关系。它的核心作用是**解决 UI 组件的 DOM 挂载问题**，尤其是在处理模态框、通知、全局弹窗等需要脱离当前组件层级的场景时非常有用。

## Teleport 的作用
1. **跨 DOM 层级渲染**
    - 将组件的内容渲染到任意 DOM 节点（如 `body` 或其他容器），避免因父组件的 CSS 限制（如 `overflow: hidden` 或 `z-index`）导致样式问题。
2. **逻辑与 DOM 解耦**
    - 组件的逻辑和状态仍保留在当前组件中，但 UI 可以渲染到其他位置，保持代码的可维护性。
3. **动态控制目标容器**
    - 支持通过条件渲染动态切换挂载目标。
...
## 核心特性
### `to` 属性
+ 指定目标容器的 DOM 选择器（如 `#app`、`.container`）或 `body`。
    - 目标容器必须**已存在**（通常是静态的 DOM 节点）。

### 禁用 Teleport
+ 通过 `:disabled="true"` 可以临时禁用传送，内容会保留在原位置渲染
...
### 多 Teleport 到同一目标
+ 多个 `<Teleport>` 可以共享同一个目标容器，内容会按声明顺序追加
...
### 复杂布局控制
+ 例如将侧边栏内容渲染到外部容器，但逻辑仍由当前组件管理。

## 注意事项
### 目标容器的存在性
+ 目标 DOM 节点必须在 `<Teleport>` 渲染前存在，否则内容不会显示。

### SSR 兼容性
+ 在服务端渲染（SSR）时，`<Teleport>` 的内容会直接渲染在原位置，客户端再动态移动到目标容器。

### 与 Transition 结合
+ 如果需要动画，将 `<Transition>` 包裹在 `<Teleport>` 外部
...
## 与 Vue 2 的对比
+ Vue 2 中类似功能需要通过第三方库（如 `portal-vue`）实现，而 Vue 3 将其标准化为内置组件，性能更高且更易用。
...
<body>
  ...
  <div id="about"></div>
  <script type="module">
    // import { ref, render, h, Text, Fragment, Teleport } from '../../node_modules/vue/dist/vue.esm-browser.js'
