# 实现
+ `Promise.all`  方法表示等待所有的 `promise` 全部成功后，会执行回调，如果有一个 `Promise`  失败则整个 `Promise` 就失败。
...
const p1 = Promise.promisify(fs.readFile)('../data/data.json', 'utf-8')
const p2= Promise.promisify(fs.readFile)('../data/data2.json', 'utf-8')

// Promise.all 方法表示等待所有的promise全部成功后，会执行回调，如果有一个 Promise 失败则 整个Promise就失败
Promise.all([1, 2, 3, p1, p2]).then(values => {
  console.log(values)
}).catch(err => {
  console.log(err)
})
