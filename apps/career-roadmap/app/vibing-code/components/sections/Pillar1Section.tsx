...
export function Pillar1Section({ active }: { active: boolean }) {
  return (
    <section className={`section ${active ? 'active' : ''}`}>
      ...
      <SectionGroup title="为什么需要配置" accent="var(--teal)">
...
// 都会被 shell 脚本拦截，stderr 反馈给 Claude`}
      ...
      <SectionGroup title="项目配置结构" accent="var(--blue)">
...
└── src/`} />
      ...
      <h3 className="section-title">Monorepo + 团队配置结构</h3>
      <Accordion title="展开查看：Monorepo 项目的分层配置方案" accent="var(--teal)">
        <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.7, marginBottom: 12 }}>
          Monorepo 的核心问题：<strong>哪些配置放根目录共享，哪些放子项目隔离？</strong>答案取决于「这条规则是否适用于所有子项目」。
        </div>
        <CodeBlock lang="tree" code={`my-monorepo/
│
│ ━━━ 根目录：全局共享 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
│
...
│   │                                 权限白名单、全局 Hooks
│   ├── settings.local.json         # 个人覆盖（不入 Git）
│   │                                 调试开关、个人偏好
│   ├── rules/                      # 全局规则（子项目可 @import）
│   │   ├── monorepo-boundary.md    #   跨应用 import 禁令
│   │   ├── git-convention.md       #   提交规范、分支策略
...
│
├── docs/                           # 全局知识库
│   ├── architecture.md             # Monorepo 整体架构
│   ├── api-contract.md             # 跨应用接口契约
│   └── decisions/                  # 全局 ADR
│
│ ━━━ 子项目：各自隔离 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
│
...
└── packages/
    └── shared/                     # 共享包
        ...
        <div style={{ marginTop: 12, padding: '12px 16px', background: 'var(--teal-bg)', borderRadius: 8, borderLeft: '3px solid var(--teal)', fontSize: 12, color: 'var(--text2)', lineHeight: 1.7 }}>
          <strong style={{ color: 'var(--teal)' }}>CLAUDE.md 读取规则：</strong>Claude 启动时会读取<strong>当前工作目录</strong>下的 CLAUDE.md，并<strong>向上递归</strong>读取父目录的 CLAUDE.md，直到项目根目录。<br />
          所以在 <code>apps/web/</code> 下工作时，Claude 会同时读取 <code>apps/web/CLAUDE.md</code> + 根目录 <code>CLAUDE.md</code>，两份规则<strong>叠加生效</strong>，子项目规则优先。
        </div>
        <div className="table-wrap" style={{ marginTop: 12 }}>
          <table>
            <thead>
              <tr><th>配置内容</th><th>放哪里</th><th>理由</th></tr>
            </thead>
            <tbody>
              {[
                ['Monorepo 边界约束（禁止跨应用 import）', '根 CLAUDE.md', '所有子项目都必须遵守'],
                ['Git 提交 / 分支规范', '根 CLAUDE.md', '团队统一，不分项目'],
                ['通用编码规范（命名、文件结构）', '.claude/rules/shared-coding.md', '可复用，按需 @import'],
                ['React 组件规范 / Hooks 规则', 'apps/web/CLAUDE.md', '仅前端适用'],
                ['数据库事务 / API 安全约束', 'apps/api/CLAUDE.md', '仅后端适用'],
                ['权限白名单 / 全局 Hooks', '.claude/settings.json', '团队共享，入 Git'],
                ['个人调试开关 / 实验性配置', '.claude/settings.local.json', '个人偏好，不入 Git'],
              ].map(([content, location, reason]) => (
                <tr key={content as string}>
                  <td style={{ fontSize: 12 }}>{content}</td>
                  <td><code style={{ fontFamily: 'var(--mono)', fontSize: 11 }}>{location}</code></td>
                  <td style={{ fontSize: 12, color: 'var(--text2)' }}>{reason}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6, lineHeight: 1.6 }}>用途：项目背景、技术栈、禁止事项、编码规范。子项目 CLAUDE.md 与根目录叠加生效，子项目优先。</div>
        </div>
        <div>
...
**/*.zip`} />
      ...
      <SectionGroup title="CLAUDE.md 编写指南" accent="var(--amber)">
      <h3 className="section-title">CLAUDE.md 的三种创建方式</h3>
      <div className="table-wrap" style={{ marginBottom: 16 }}>
        <table>
          <thead>
            <tr><th>方式</th><th>命令 / 操作</th><th>适合场景</th><th>推荐度</th></tr>
          </thead>
          <tbody>
            {[
            ...
            ].map(([method, cmd, scene, rec]) => (
              <tr key={method as string}>
                <td style={{ fontWeight: 600, fontFamily: 'var(--mono)', fontSize: 12, whiteSpace: 'nowrap' }}>{method}</td>
                <td><code style={{ fontFamily: 'var(--mono)', fontSize: 11 }}>{cmd}</code></td>
                <td style={{ fontSize: 12 }}>{scene}</td>
                <td style={{ fontSize: 12, fontWeight: 500 }}>{rec}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <CompareBlock
        badLabel="❌ /init 后直接用（常见错误）"
        goodLabel="✅ /init 后手动精简（正确做法）"
        bad={`# /init 生成了 200 行 CLAUDE.md
# 包含大量通用模板内容：
#   - "使用 TypeScript"
#   - "遵循最佳实践"
#   - "编写清晰的代码"
#
# 问题：这些是废话，删掉 Claude 也不会犯错
# 结果：150+ 行的噪音淹没了真正重要的规则`}
        good={`# /init 生成后，逐行审查：
# "删掉这行，Claude 会犯什么具体错误？"
#
# ✅ 保留：
- 禁止引入新 UI 库（已有 antd）
- 禁止直接 import axios → 用 apis/request.js
- Less 嵌套规则须在父选择器 {} 内
#
# ❌ 删除：
- 使用 TypeScript（tsconfig 已约束）
- 编写清晰的代码（废话）
#
# 最终：精简到 48 行，每行都有防御价值`}
      />
      <div style={{ padding: '12px 16px', background: 'var(--amber-bg)', borderRadius: 8, borderLeft: '3px solid var(--amber)', fontSize: 12, color: 'var(--text2)', lineHeight: 1.7, marginBottom: 20 }}>
        <strong style={{ color: 'var(--amber)' }}>关键区分：</strong><code>/init</code> 是<strong>扫描工具</strong>，不是写作工具。它帮你发现项目结构和技术栈，但生成的内容需要你用「黄金法则」逐行过滤。直接用 /init 的输出，等于把一份未经审核的草稿当作正式文档。
      </div>
...
| [\`docs/coding-style.md\`](docs/coding-style.md) | 命名规范、禁止事项、常见陷阱 |`} />
      ...
      <SectionGroup title="Hooks 与 Skills" accent="var(--purple)">
...
---`} />
  ...
  In/Out       写入decisions/   写入CURRENT.md   每任务更新状态   AC(验收准则)覆盖矩阵  Critical归零方可合并  文档传导`} />
...
- 可追溯链路：REQ(需求) → ADR(架构决策) → Tasks(任务) → TCs(测试用例) → CRs(问题)（全部 Closed）
...
git worktree remove ../project-frontend`} />
      ...
      <SectionGroup title="操作技巧与效率" accent="var(--amber)">
      <h3 className="section-title">Effort Level 思维档位</h3>
