...
<!-- 这是一张图片，ocr 内容为：安全性 LIGHTHOUSE PERFORMANCE NETWORK WELCOME CONSOLE APPLICATION ELEMENTS MEMORY SOURCES CONTENT-WEBKIT"> <META NAME-RENDERER CON <TITLE>淘宝网 -淘!找喜欢</TITLE> <META NAME-"SPM-ID" CONTENT-"A21BO"> 并由商家提供退货承诺,破损补寄等消费者保障服务,让你安心享受网上购物乐趣!"> <META NAME-"APLUS-XPLUG" CONTENT-"NONE"> 拍卖,网上开店,网络购物,打折,免费开店,网购,频道,店铺"> <LINK REL-"DNS-PREFETCH" HREF HREF-"/G.ALICDN.COM"> HREF-"/IMG.ALICDN.COM"> <LINK REL-"DNS-PREFETCH" <LINK REL-"DNS-PREFETCH" HREF-"/TCE.ALICDN.COM"> <LINK REL-"DNS-PREFETCH" HREF-"/GM.MMSTAT.COM"> <LINK REF-"DNS-PREFETCH"HRE HREF-"/TCE.TAOBAO.COM" <LINK REL-"DNS-PREFETCH" HREF HREF//LOG.MMSTAT.COM <LINK REL-"DNS-PREFETCH" HREF-"/TUI.TAOBAO.COM"> <LINK REL-"DNS-PREFETCH" HREF-"/ALD.TAOBAO.COM"> <LINK REL-"DNS-PREFETCH"HREF-"/GW.ALICDN.COM"> <LINK REL-"DNS-PREFETCH" HREF-"/ATANX.ALICDN.COM"> <LINK REL-"DNS-PREFETCH" HREF-"/DFHS.TANX.COM"> <LINK REL-"DNS-PREFETCH" HREF-"/ECPM.TANX.COM"> <LINK REL-"DNS-PREFETCH" HREF-"/RES.MMSTAT.COM"> <STYLE>.</STYLE> <SCRIPT>.</SCRIPT>  <BASE TARGET-" BLANK"> <STYLE>.</STYLE> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672740421054-ed6f2b9c-7b86-4630-bffa-49f0a28bd1c6.png)
...
+ 每当站点引用跨域域上的资源时，都应在 <head> 元素中放置 dns-prefetch 提示，但是要记住一些注意事项。
    - （1）dns-prefetch 仅对 [跨域](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/CORS) 域上的 DNS 查找有效，因此请避免使用它来指向您的站点或域。这是因为，到浏览器看到提示时，您站点域背后的IP已经被解析。
...
+ [X-DNS-Prefetch-Control - HTTP | MDN](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-DNS-Prefetch-Control)

# 三、HTTP 长连接
## 短连接
+ HTTP 协议的初始版本中，每进行一次 HTTP 通信就要断开一次 TCP 连接。

<!-- 这是一张图片，ocr 内容为：建立TCP连接 SYN SYN/ACK ACK 00 HTTP请求 HTTP响应 FIN 客户端 服务器 ACK FIN ACK 断开TCP连接 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672741171335-642c0cfa-2aa5-4c55-82ee-3b8d0e69f9c2.png)
...
<!-- 这是一张图片，ocr 内容为：BOOKS 发送请求一份包含多张图 片的HTML文档对应的 WEB页面,会产生大量的 通信开销. 获取HTML文档 建立TCP连接 必须进行多次通信. 好累 HTTP请求/响应 获取图片 断开TCP连接 建立TCP连接 HTTP请求/响应 断开TCP连接 客户端 服务器 获取图片 建立TCP连接 HTTP请求/响应 断开TCP连接 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672741208108-bc045387-2ffa-405c-bcb6-f4af3819ee16.png)
...
<!-- 这是一张图片，ocr 内容为：建立TCP连接 SYN SYN/ACK 只要建立连接就能 ACK WEB 页 面的打开 一次性发送请求的 速度变快了! HTTP请求 资源了 HTTP响应 HTTP请求 HTTP响应 00 HTTP请求 客户端 HTTP响应 服务器 FIN ACK FIN ACK 断开TCP连接 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672741368792-1bec518a-2074-4805-a04e-8407d898c022.png)
...
<!-- 这是一张图片，ocr 内容为：不用等待就能直接 发送下一个请求! 建立TCP连接 HTTP请求1 00 HTTP请求 HTTP响应1 HTTP响应2 客户端 服务器 断开TCP连接 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672741519850-d00995b6-b38d-43aa-9b3c-523bd0d0e58e.png)
...
+ 这是一个对比 HTTP1 和 HTTP2 资源加载的在线示例：[HTTP/2: the Future of the Internet | Akamai](https://http2.akamai.com/demo)
...
<!-- 这是一张图片，ocr 内容为：CLIENT SERVER INITIAL REQUEST GET/DOCHTTP/1.1 RESOURCE MOVED. HTTP/1.1 301 MOVED NEW LOCATION RETURNES PERMANENTLY LOCATION: /DOC NEW REQUEST TO THE NEW LOCATION GET /DOC NEW HTTP/1.1 RESOURCE RETURNED. 200 HTTP/1.1 OK -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672755038580-ea34044a-39cf-418a-8061-db26b411c31b.png)
...
| 编码 | 含义 | 处理方法 | 典型应用场景 |
| --- | --- | --- | --- |
| 301 | Moved Permanently | [GET ](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Methods/GET)方法不会发生变更，其他方法有可能会变更为 GET 方法 | 网站重构 |
...
| 编码 | 含义 | 处理方法 | 典型应用场景 |
| --- | --- | --- | --- |
| 302 | Found | [GET ](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Methods/GET)方法不会发生变更，其他方法有可能会变更为 GET 方法 | 由于不可预见的原因该页面暂不可用。在这种情况下，搜索引擎不会更新它们的链接 |
...
+ 开始之前，先来介绍本文涉及的三种数据压缩格式：
    - DEFLATE，是一种使用 Lempel-Ziv 压缩算法（LZ77）和哈夫曼编码的压缩格式。详见 [RFC 1951: DEFLATE Compressed Data Format Specification version 1.3](https://www.rfc-editor.org/rfc/rfc1951)
    - ZLIB，是一种使用 DEFLATE 的压缩格式，对应 HTTP 中的 Content-Encoding: deflate。详见 [RFC 1950: ZLIB Compressed Data Format Specification version 3.3](https://www.rfc-editor.org/rfc/rfc1950)
    - GZIP，也是一种使用 DEFLATE 的压缩格式，对应 HTTP 中的 Content-Encoding: gzip。详见 [RFC 1952: GZIP file format specification version 4.3](https://www.rfc-editor.org/rfc/rfc1952)
...
var bufBody = new Unit8Array(rawLen);
...
## 参考链接
+ [HTTP 协议中的数据压缩 - HTTP | MDN](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Compression)
...
+ HTTP 缓存应该算是前端开发中最常接触的缓存机制之一，它又可细分为**<font style="color:#DF2A3F;">强制缓存与协商缓存</font>**，二者最大的区别在于判断缓存命中时，浏览器是否需要向服务器端进行询问以协商缓存的相关信息，进而判断是否需要就响应内容进行重新请求。下面就来具体看HTTP缓存的具体机制及缓存的决策策略。
...
## 协商缓存
+ 顾名思义，**<font style="color:#DF2A3F;">协商缓存就是在使用本地缓存之前，需要向服务器端发起一次 GET 请求，与之协商当前浏览器保存的本地缓存是否已经过期</font>**
...
### last-modifed 的不足
+ 通过 last-modifed 的不足 所实现的协商缓存能够满足大部分的使用场景，但也存在两个比较明显的缺陷：
...
### 缓存决策树
<!-- 这是一张图片，ocr 内容为：是否使用缓存 是 NO-STORE 是否进行 协商缓存 否 NO-CACHE 是否会被 代理服务器 缓存 PRIVATE PUBLIC 配置强制缓存 过期时间 配置协商缓存的 ETAG或LAST-MODIFIED 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672842377284-1519cc61-923c-484e-aff3-4d2a1c7fda8e.png)
...
<!-- 这是一张图片，ocr 内容为：注册 安装过程 安装 激活 失败 激活过程 响应缓存 替换为新的 SERVICE WORKER 更新 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672844183744-272ed1a5-09cb-4a55-b128-c85ad851c7f4.png)
...
<!-- 这是一张图片，ocr 内容为：WORKER LIFECYCLE INSTALLING REGISTRATION THIS STAGE MARKS THE BEGINNING OF REGISTRATION. IT'S INTENDED TO ALLOW TO SETUP WORKER-SPECIFIC RESOURCES SUCH AS OFFLINE CACHES. INSTALLING WAITING ACTIVE INSTAL1 USE EVENT.WAITUNTILO PASSING A PROMISE TO EXTEND THE INSTALLING STAGE UNTIL THE PROMISE IS RESOLVED. USE SELF SKIPWAITINGO ANYTIME BEFORE ACTIVATION TO SKIP INSTALLED STAGE AND DIRECTLY JUMP TO ACTIVATING STAGE WITHOUT WAITING FOR CURRENTLY CONTROLLED CLIENTS TO CLOSE. INSTALLED REGISTRATION THE SERVICE WORKER HAS FINISHED ITS SETUP AND IT'S WAITING FOR CLIENTS USING OTHER SERVICE WORKERS TO BE CLOSED. WAITING ACTIVE INSTALLING ACTIVATING REGISTRATION THERE ARE NO CLIENTS CONTROLLED BY OTHER WORKERS. THIS STAGE IS INTENDED TO ALLOW THE WORKER TO FINISH THE SETUP OR CLEAN OTHER WORKER'S RELATED ACTIVE WAITING INSTALLING RESOURCES LIKE REMOVING OLD CACHES. ACTIVATE USE EVENT.WAITUNTITO PASSING A PROMISE TO EXTEND THE ACTIVATIVATING STAGE UNTIL THE PROMISE IS RESOLVED. USE SELF.CLIENTS.CLAIMO IN THE ACTIVATE HANDLER TO START CONTROLLING ALL OPEN CLIENTS WITHOUT RELOADING THEM. ACTIVATED REGISTRATION 口 THE SERVICE WORKER CAN NOW HANDLE FUNCTIONAL EVENTS. ACTIVE WAITING INSTALLING REDUNDANT REGISTRATION THIS SERVICE WORKER IS BEING REPLACED BY ANOTHER ONE. ACTIVE WAITING INSTALLING 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672844228144-3161f0bb-325c-479c-a795-d1c92f1ef6da.png)
...
<!-- 这是一张图片，ocr 内容为：安全性 ELEMENTS LIGHTHOUSE PERFORMANCE NETWORK APPLICATION MEMORY SOURCES WELCOME CONSOLE #765 ACTIVATE APPLICATION MANIFEST HTTP://127.0.1:5500/SW-TEST2/ SERVICE WORKERS STORAGE SOURCE 4 SW.JS STORAGE 接收时间2021/5/27下午4:57:37 LOCAL STORAGE #764已激活并已正在运行停止 STATUS 会话存储 INDEXEDDB 接收时间2021/5/27下午5:43:11 #772等待激活SKIPWAITING WEB SQL COOKIE 客户端 HTTP://127.0.1:5500/SW-TEST2/INDEX.HTML 焦点 TRUST TOKENS 推送 测试来自DEVTOOLS的推送消息. 推送 CACHE 同步 同步 TEST-TAG-FROM-DEVTOOLS 缓存存储 APPLICATION CACHE 定期同步 定期同步 TEST-TAG-FROM-DEVTOOLS BACKGROUND SERVICES 更新周期 更新活动 TIMELINE VERSION 后台获取 拉勾教育 #764 INSTALL 后台同步 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672883151061-78eb65be-fc50-4581-99b2-c9a921d22827.png)
...
<!-- 这是一张图片，ocr 内容为：PERFORMANCE 安全性 WELCOME LIGHTHOUSE MEMORY NETWORK ELEMENTS SOURCES APPLICATION CONSOLE APPLICATION SERVICE WORKERS MANIFEST OFFLIHE 网络旁路 重新加载时更新 SERVICE WORKERS STORAGE HTTP://127.0.1:5500/SW-TEST2/ STORAGE SOURCE SWIS LOCAL STORAGE 会话存储 接收时间2021/5/27下午6:53:06 INDEXEDDB #820已激活并已正在运行停止 STATUS WEB SQL COOKIE HTTP://127.0.1:5500/SW-TEST2/INDEX.HTML 焦点 客户端 TRUST TOKENS 推送 推送 测试来自 DEVTOOLS 的推送消息. CACHE 缓存存储 同步 同步 TEST-TAG-FROM-DEVTOOLS APPLICATION CACHE 定期同步 定期同步 TEST-TAG-FROM-DEVTOOLS 拉勾教育 BACKGROUND SERVICES 更新周期 更新活动 TIMELINE VERSION -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672884649256-b95704be-b306-4e42-a445-2a9a31d9924d.png)
...
<!-- 这是一张图片，ocr 内容为：3.返回响应 页面 缓存 1.请求拦截 5.更新/存储缓存 2.提取缓存 -4.检查更新 网络 SERVICE WORKER 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672884861083-bb5faf48-28fe-4bd1-ad2f-fd51dc8c60e9.png)
...
+ [GitHub - mdn/sw-test: Service Worker test repository. This is a very simple demo to show basic service worker features in action.](https://github.com/mdn/sw-test)
...
<!-- 这是一张图片，ocr 内容为：. 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672885574143-0cbde536-3a24-4d7c-bcc6-c5920e19042b.png)

+ 如果未使用 CDN 网络进行缓存加速，那么通过浏览器访问网站获取资源的大致过程如图所示。

<!-- 这是一张图片，ocr 内容为：1.请求域名解析 2.返回域名对应的服务器IP 3.根据IP请求服务器内容 DNS SERVER 4.服务器返回响应资源 WEB SERVER 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672885600737-1d78462b-274a-40bf-83da-e682eec2725f.png)
...
<!-- 这是一张图片，ocr 内容为：2.域名解析 CDN专用 DNS SERVER DNS SERVER 1.请求域名解析 3.CDN负载均衡器IP 4.请求资源 5.返回分配的目标缓存服务器 CDN负载均衡器 7.服务器返回响应资源 6.请求资源 WEB SERVER 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672885671370-a2e66ce9-81d3-4424-ba67-9f24387d2afd.png)
...
<!-- 这是一张图片，ocr 内容为：HTTPS//WWW.TAOBAO.COM 免费开店 千牛卖家中心 我的淘宝 购物车0收藏夹商品分类 `联系客服 手机逛淘宝 中国大陆 网站导航 亲,请登录免费注册 去领红包 淘宝 搜索 宝贝 高端连衣裙炸街裙子 TAOBAO 新款连衣裙四件套潮波T他时尚女鞋短裤半身裙男士外套墙纸行车记录仪新款男鞋耳机时尚女包沙发 聚划算 主题市场 智能生活 天猫超市I司法拍卖飞猪旅行天天特卖淘小铺|造点新货苏宁易购淘 易购 淘宝心选 天猫 女装/内衣/家居 好车捡漏 女鞋/男鞋/箱包 HI!你好 1元起拍 母婴/童装/玩具 抢稀缺资源 领淘金币抵钱 会员俱乐部 男装/运动户外 注册 登录 开店 马上去抢 投资土地买厂房 关妆/彩妆/个护 淘宝逛逛邀您一起 分享精彩生活 手机/数码/企业 速来查看> 大家电/生活电器 公益 千牛 安全 规则 公告 零食/生鲜/茶酒 新手开店 千牛头条 厨具/收纳/清洁 TMALL天猫理想生活上天猫 1/5今日热卖 品质 家纺/家饰/鲜花 好货 旅行 充话费 游戏 车险 名店潮流每日上新 名店潮搭 猜你 新品爆款 图书音像/文具 喜欢 5折起 件件精品 新品不断 旅行险 财产险 车险 意外险 健康险 医药保健/进口 热门城市 请选择城市 反馈 汽车/二手车/用品 保险公司 人保车险 暴恐 房产/装修家具/建材> 举报 立刻报价 车险活动 快速续保 手表/眼镜/珠宝饰品> 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672885939406-717ca896-ea96-4daf-8597-f76bd085e27a.png)
...
<!-- 这是一张图片，ocr 内容为：DEVTOOLS WWW.TAOBAO.COM/ 安全性 MEMORY PERFORMANCE APPLICATION LIGHTHOUSE CONSOLE ELEMENTS NETWORK SOURCES 74 WELCOME O O DISABLE CACHE NO THROTTLING PRESERVE LOG FILTER 隐藏数据 URL 已阻止请求 已阻止COOKIE 文档WS XHR JS CSS IMG MANIFEST MEDIA FONT OTHER 22000 MS 26000 MS 2000 MS 12000 MS 28000 MS 16000 MS 14000 MS 24000 MS 18000 MS 4000 MS 6000 MS 8000 MS 20000 MS 10000 MS NAME 计时 HEADERS COOKIE X PREVIEW RESPONSE INITIATOR WWW.TAOBAO.COM ??TBHOME/TAOBAO-2021/0.0.4/ 商品: 我的淘宝口购物车口收藏夹 淘宝网首页 APLUS_V2.JS 淘宝网 S.GIF 淘宝网 O1CN01GIZLK91HIWVNTOLEQ_!! 宝贝 77KISSY/K/6.2.4/SEED-MIN.JS,KG/ 天猫 ??UNIVERSAL/POLYFILL/0.0.2/P/IND 店铺 ??KISSY/K/6.2.4/EVENT-CUSTOM- 搜索 ??TBHOME/TAOBAO-2021/0.0.4/. FONT_1404888168_2057645.WO FONT_403341_N8TJ33YN5PENG6. 选择文件 未选择文件 O1CN01PAQURJ1QGNAICTCGG. O1CN01PAQURJ1QGNAICTCGG. 新款连衣裙四件套潮流丁恤时尚女鞋短裤半身裙男士外套墙纸行车记录仪新款男鞋耳机时尚女包沙发 O1CN01URZ3DE23MZWOFMPY. 去领红包 TB1BLOBNFXXXXXYXXXXXXXXXX. 主题市场 TB1EIXTXITH8KJJYOFIXXCRSXX.. 天猫 TB1EIXTXITH8KJJY0FIXXCRSXX. 聚划算 天猫超市 TB1EIXTXITH8KJJYOFIXXCRSXX. 拉勾教育 107次请求已传输87.3 KB 2.3 MB -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672885960467-0947887c-e1be-4b91-ad80-2ebe68e78c11.png)
...
<!-- 这是一张图片，ocr 内容为：客户端页面1 客户端页面2 图片,预加载 图片,预加载 等本地缓存 等本地缓存 SERVICE WORKER 缓存 HTTP缓存 HTTP2连接 HTTP 2连接 PUSH缓存 PUSH缓存 服务器 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672886372031-7b9da1d0-eceb-4a67-af0c-c65829551533.png)
...
<!-- 这是一张图片，ocr 内容为：资源文件 是否为当前 一是一 否 页面所需 是否为 是 第三方资源 是否使用 一是 否 SERVICE WORKER 是否利用 仅是为了更快 服务器空闲时间 加载资源 加载内联资源 是 确定要这么做 预加载 PUSH缓存 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1672886697113-f81cd03e-4e24-4a04-bc56-2bbc9760d6bf.png)
...
# 十一、使用服务端渲染
+ [服务端渲染相关概念](https://www.yuque.com/lotosv2010/front_end/tbkuac4zw3598808)
