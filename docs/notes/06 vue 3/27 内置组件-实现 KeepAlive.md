# 简介
+ 在 Vue 3 中， `[**<KeepAlive>**](https://cn.vuejs.org/guide/built-ins/keep-alive.html)` 是一个**内置组件**，用于缓存动态组件的状态，避免重复渲染和销毁。它的核心作用是**优化性能**，特别是在需要频繁切换组件的场景中。

## KeepAlive 的作用
1. **缓存组件状态**
    - 当组件被切换时， `<KeepAlive>` 会缓存其状态（如数据、DOM 结构等），而不是销毁它。下次切换回来时，直接从缓存中恢复，避免重新渲染和初始化。
2. **减少性能开销**
    - 适用于需要保留用户输入、表单数据或复杂计算的组件（如标签页、路由视图等），避免重复执行 `created` 和 `mounted` 生命周期钩子。
3. **支持条件性缓存**
    - 可以通过 `include` 、 `exclude` 或 `max` 属性精确控制哪些组件需要缓存。
...
### 生命周期钩子
+ 缓存的组件会触发特有的生命周期钩子：
    - `activated` ：组件被激活（从缓存中恢复时调用）。
    - `deactivated` ：组件被停用（进入缓存时调用）。

### 缓存控制
+ `include` ：指定需要缓存的组件名（字符串或正则）。
+ `exclude` ：指定不需要缓存的组件名。
+ `max` ：限制最大缓存组件数量（超出时会销毁最久未使用的组件）。
...
## 使用场景
1. **标签页切换**
    - 缓存每个标签页的内容，避免切换时丢失数据或重新加载。
2. **表单保留**
    - 用户填写表单时，临时切换到其他页面后返回，表单数据不会丢失。
3. **复杂计算优化**
    - 缓存计算结果或大数据渲染的组件，减少重复计算。
4. **路由页面缓存**
    - 缓存某些高频访问的页面（如首页、详情页）。

## 注意事项
1. **内存占用**
    - 缓存过多组件会增加内存消耗，需合理设置 `max` 属性。
2. **状态一致性**
    - 缓存的组件不会重新触发 `created` 或 `mounted` ，需通过 `activated` 更新数据。
3. **动态组件名**
    - 如果组件是动态注册的（如异步组件），需确保 `include` 或 `exclude` 能匹配到名称。
...
<body>
  <div id="app"></div>
  <script type="module">
    // import { ref, render, h, provide, inject, reactive, getCurrentInstance, KeepAlive, onMounted } from '../../node_modules/vue/dist/vue.esm-browser.js'
