...
+ 其中LQIP的全称是低质量图片占位符，即使用原图的较低分辨率版本来占位，SQIP则是一种基于SVG的LIQP技术，我们可以通过对比来感知它们和原图之间的差别，如图4.5所示。

<!-- 这是一张图片，ocr 内容为：教育 艺术的SD 基于像素块的SQIP LQIP SQIP 原图 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673228872693-85c0d125-0f87-40fb-bc12-d3fa26939fb5.png)
...
<!-- 这是一张图片，ocr 内容为：NEW IMAGE(); CONST NEWIMAGE NEWIMAGE.SRC - "PHOTO.JPG"; //当发生故障时的处理措施 (ERR) -> { NEWIMAGE.ONERROR //图像加载后的回调 NEWIMAGE.ONLOAD D二)()>{ 拉勾教育 子; -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673228931237-f97ca70b-d0a3-4dee-8130-aae3e35731f9.png)
...
<!-- 这是一张图片，ocr 内容为：<BUTTON ID-"LOAD-IMAGE">加载图像</BUTTON> <DIV ID-"IMAGE-CONTAINER"> 拉勾教育 </DIV> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229004380-60fc0fe2-dda3-4b95-85e3-aa7e9b6a1df3.png)

+ 对应的JavaScript事件处理代码如下：

<!-- 这是一张图片，ocr 内容为：DOCUMENT.ADDEVENTLISTENER("DOMCONTENTLOADED", DOCUMENT.GETELEMENTBYID("LOAD-IMAGE"); CONST LOADBUTTON DOCUMENT.GETELEMENTBYID("IMAGE-CONTAINER"); IMAGECONTAINER CONST NEWIMAGE - NEW IMAGE(); CONST "HTTPS://XX.CDN/VERY-BIG-PHOTO.JPG"; NEWIMAGE.SRC LOADBUTTON.ADDEVENTLISTENER("CLICK", FUNCTION() () ( IF ("DECODE" IN NEWIMAGE) ] 拉勾教育 //异步解码方式 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229020459-b9ea7e9e-9ac4-46a7-8d85-b453cf904373.png)

<!-- 这是一张图片，ocr 内容为：NEWIMAGE.DECODE().THEN(FUNCTION() () ( IMAGECONTAINER.APPENDCHILD(NEWIMAGE); }); ELSE //正常图像加载方式 IMAGECONTAINER.APPENDCHILD(NEWIMAGE); TRUE ONCE: 厂) 拉勾教育 了) -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229026601-418392f7-52f1-4330-a975-57d1d613d5ba.png)
...
<!-- 这是一张图片，ocr 内容为：<!-使用延迟加载的图像文件标签--> SRC-"PLACEHOLDER-IMAGE.JPG" DATA-SRC"IMAGE-TO-LAZY CLASS"LAZY" <IMG LOAD.JPG" ALT"MAN AN IMAGE!"> <!-当JAVASCRIPT不可用时,原生展示目标图像--> <NOSCRIPT> IMAGE!"> <IMG "IMAGE-TO-LAZY-LOAD.JPG" ALT-"I'M AN SRC"IN 拉勾教育 </NOSCRIPT> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229068261-37ee370e-7b01-4e69-a00e-ac54c7d9639b.png)

+ 如果上述代码同时存在，当JavaScript不可用时，页面中会同时展示图像占位符和〈noscript〉中包含的图像，为此我们可以给〈html〉标签添加一个no-js类：

<!-- 这是一张图片，ocr 内容为：拉勾教育 <HTML CLASS"NO-JS"> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229084861-689151b0-11dc-462e-bfc8-917bc187016e.png)

+ 在由〈link〉标签请求CSS文件之前，在〈head〉标签结构中放置一段内联脚本，当JavaScript可用时，用于移除no-js类：

<!-- 这是一张图片，ocr 内容为："NO-JS");</SCRIPT拉勾教育 <SCRIPT>DOCUMENT.DOCUMENTELEMENT.CLASSLIST.REMOVE("NO-JS" -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229102315-45665224-1902-4fdd-8c8e-7a1decd49e1b.png)

+ 以及添加必要的CSS样式，使得在JavaScript不可用时屏蔽包含.lazy类元素的显示：

<!-- 这是一张图片，ocr 内容为：.NO-JS .LAZY { DISPLAY: NONE; 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229122615-bf280228-d47a-4af7-aa94-7cebdeb6a2ce.png)
...
+ 比如，在 <head> 标签中，CSS 文件通常具有最高的优先级 Highest，其次是 \x3Cscript> 标签所请求的脚本文件，但当 \x3Cscript> 标签带有 defer 或 async 的异步属性时，其优先级又会降为 Low。我们可以通过 Chrome 的开发者工具，在 Network 页签下找到浏览器对资源进行的优先级划分，如下图所示。

<!-- 这是一张图片，ocr 内容为：NETWORK APPLICATION SECURITY SOURCES CONSOLE AUDITS PERFORMANCE ELEMENTS MEMORY DISABLE CACHE Q ONLINE PRESERVE LOG 280000 MS 240000 MS 260000MS 160000 MS 180000MS 220000 MS 120000 MS 140000 MS 200000 MS 60000 MS 20000 MS 40000 MS 80000 MS 300000 100000 MS STATUS TYPE TIME WATERFA PRIORITY INITIATOR SIZE NAME 200 KFOKCNGEU92FR1MU51XLLZIXKMNY.WOFF2 FONT (SERVICEWORKER) 4 MS HIGHEST RESOURCE-PRITIZATION?HLZHLZH-CN @ KFOKCNGEU92FR1MU51XLLZIXKMNY.WOFF2 200 (DISK CACHE) FETCH 1 MS HIGHEST SW.JS:62 (SERVICEWORKER) 200 SCRIPT 3 MS WEBCOMPONENTS-LITE,JS LOW APP LOADER.JS:75 (SERVICEWORKER) DEVSITE_APP_ZH_CN.JS 200 5MS SCRIPT LOW APP LOADER.JS:75 (SERVICEWORKER) 4 MS DEVSITE APP ZH CN.JS:189 LOW 200 SCRIPT ANALYTICS.JS 1 MS FETCH 200 (DISK CACHE) LOW SW.JS:62 ANALYTICS.JS 5MS KFOICNQEU92FR1MMEU9FBBC4AMP6IQ.WOFF2 HIGHEST 200 (SERVICEWORKER) DEVSITE APP ZH CN.JS:215 FONT 2 MS HIGHEST 卷 KFOICNGEU92FR1MMEU9FBBC4AMP6IQ.WOFF2 200 (DISK CACHE) SW.JS:62 FETCH 489 MS HIGH 20.1 KB DEVSITE APP ZH CN.JS:192 200 FETCH OGB?HLZH-CN 4 MS LOW (SERVICEWORKER) 200 LINKID.JS ANALYTICS.JS:68 SCRIPT 200 1 MS (DISK CACHE) FETCH SW.JS:62 LINKID.JS 拉勾教育 (SERVICEWORKER) 246MS LOW COLLECT7V-18_V-1798A-38911766&T-PAGEVIEW&S-18DL-..NTA....200 ANALYTICS.IS:16 GIF -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229295751-b643db97-a88a-4e41-9517-b748f52911b3.png)
...
<!-- 这是一张图片，ocr 内容为：CONSOLE ANIMATIONS . FILTER DEFAULT LEVELS TOP THE RESOURCE HTTP://LOCALHOST:8000/UNUSED.IS WAS PRELOADED LOCALHOST/:1 USING LINK PRELOAD BUT NOT USED WITHIN A FEW SECONDS FROM THE WINDOW'S LOAD EVENT. PLEASE MAKE SURE IT WAL WASN'T PRELOADED FOR NOTHING. 拉勾教育 -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229426745-a35304f6-64d5-4ac5-831c-4e7cacc151e6.png)
...
<!-- 这是一张图片，ocr 内容为：REL"PRELOAD" TYPE"FONT/WOFF2" AS''FONT" <LINK CROSSORIGIN-"CROSSORIGIN" 拉勾教育 HREF"MYFONT.WOFF2"> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673229456777-cb54e587-2bcd-4932-8563-af6c70d45960.png)
...
## 预连接
+ [https://developer.chrome.com/docs/lighthouse/performance/uses-rel-preconnect/](https://developer.chrome.com/docs/lighthouse/performance/uses-rel-preconnect/)
...
<!-- 这是一张图片，ocr 内容为：拉勾教育 HREF"HTTPS://EXAMPLE.COM"> REL-"PRECONNECT" <LINK -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673230202441-f3ca424c-bfa4-4a77-8645-56496fb7f034.png)
...
<!-- 这是一张图片，ocr 内容为：拉勾教育 <LINK REL-"PREFETCH" HREF-"PAGE-2.HTML"> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673230325473-fb0107b9-a1cb-429b-abd9-af5e83fde787.png)
...
<!-- 这是一张图片，ocr 内容为：<HTML <HEAD> <LINK  REL-"PREFETCH" HREF-"STYLE.CSS"> <LINK REL-"STYLESHEET" HREF-"STYLE.CSS"> </HEAD> <BODY> WORLD! HE110 </BODY> 拉勾教育 </HTML> -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673230363714-a22d7c2b-3499-4a75-a057-5ad66cbf7fe0.png)

+ 可能你会觉得对 style.css 的预提取声明，会降低接下来 <link rel="stylesheet"href="style.css"> 的优先级，但其真实的情况是，该文件会被提取两次，第二次可能会使用缓存，如下图所示。

<!-- 这是一张图片，ocr 内容为：PRIORITY WATERFALL NAME SIZE TYPE STATUS INITIATOR TIME 200 344 B DOCU... OTHER HIGHEST LOCALHOST 5 MS HIGHEST 200 212B (INDEX) 26 MS STYLES... OPTIONAL.CSS (INDEX) 200 212B 9MS LOWEST 拉勾教育 TEXT/CSS OPTIONAL.CSS -->
![](https://cdn.nlark.com/yuque/0/2023/png/738210/1673230390822-2a2ea1a1-f0eb-416a-a14a-d89c36c0f449.png)
